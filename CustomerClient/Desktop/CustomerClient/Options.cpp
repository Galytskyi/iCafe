...
void CustomerDataOption::load()
{
	...
	m_serverIP = s.value(QString("%1ServerIP").arg(CUSTOMER_DATA_REG_KEY), "127.0.0.1").toString();
	...
	m_phone = s.value(QString("%1Phone").arg(CUSTOMER_DATA_REG_KEY), 0).toULongLong();
	m_orderTime = s.value(QString("%1OrderTime").arg(CUSTOMER_DATA_REG_KEY), 0).toUInt();
