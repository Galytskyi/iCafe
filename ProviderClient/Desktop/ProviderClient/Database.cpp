...
int SqlTable::read(void* pRecord, int* key, int keyCount)
{
	...
	while (query.next() == true)
	{
		...
		switch(m_info.objectType())
		{
			...
			case SQL_TABLE_ORDER:
				{
//					Order::Item* order = static_cast<Order::Item*> (pRecord) + readedCount;

//					order->setOrderID(query.value(field++).toInt());
...
int SqlTable::write(void* pRecord, int count, int* key)
{
	...
	for (int r = 0; r < count; r++)
	{
		...
		switch(m_info.objectType())
		{
			...
			case SQL_TABLE_ORDER:
				{
					Order::Item* order = static_cast<Order::Item*> (pRecord) + r;

					//order->setOrderID(lastKey() + 1);

					query.bindValue(field++, order->handle().ID);
