...
void ProviderDataOption::load()
{
	...
	m_providerID = s.value(QString("%1ProviderID").arg(PROVIDER_DATA_REG_KEY), -1).toInt();
	m_requestProviderTime = s.value(QString("%1RequestProviderTime").arg(PROVIDER_DATA_REG_KEY), 1000).toUInt();
	m_requestCustomerTime = s.value(QString("%1RequestCustomerTime").arg(PROVIDER_DATA_REG_KEY), 1000).toUInt();

	m_serverIP = s.value(QString("%1ServerIP").arg(PROVIDER_DATA_REG_KEY), "127.0.0.1").toString();
